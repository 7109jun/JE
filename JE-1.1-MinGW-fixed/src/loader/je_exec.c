...
static size_t exec_find_alloc(const JEExecContext *ctx, const void *ptr) {
...
static int parse_string_literal(const char *s, char *out, size_t cap) {
    if (!s || !out || cap == 0 || *s != '"') return 0;
    size_t n = 0;
    int escaped = 0;
...
static JEStatus exec_line(JEExecContext *ctx, JEProgram *p, char *line, int *ip_changed) {
    ...
    if (strcmp(op, "CALL") == 0) {
        char *target_name = trim(cursor);
        if (strncmp(target_name, "IMPORT.", 7) == 0) {
            JEStatus rst = je_runtime_call(ctx, target_name);
            if (rst == JE_ERR_UNSUPPORTED && strstr(target_name, ".CRT.") == NULL) {
                char alias[96];
                if (strlen(target_name) < sizeof(alias) - 5u) {
                    snprintf(alias, sizeof(alias), "IMPORT.CRT.%s", target_name + 7);
                    rst = je_runtime_call(ctx, alias);
                }
            }
            return rst;
        }
        size_t target = find_label(p, target_name);
    ...
    if (strcmp(op, "STR") == 0) {
        ...
        if (comma) {
            *comma = '\0';
            char *dst_name = trim(arg);
            dst = parse_reg(dst_name);
            literal = trim(comma + 1);
            if (dst < 0) { je_set_error("STR destination must be a register"); return JE_ERR_PARSE; }
        }
        char decoded[JE_EXEC_LINE_MAX];
        if (!parse_string_literal(literal, decoded, sizeof(decoded))) {
            je_set_error("STR expects a string literal"); return JE_ERR_PARSE;
        }
        size_t n = strlen(decoded);
        char *copy = (char *)malloc(n + 1u);
        if (!copy) { je_set_error("STR allocation failed"); return JE_ERR_MEMORY; }
        memcpy(copy, decoded, n + 1u);
        if (ctx->alloc_count >= 128u) { free(copy); je_set_error("too many JE allocations"); return JE_ERR_MEMORY; }
        ctx->allocs[ctx->alloc_count] = copy;
        ctx->alloc_sizes[ctx->alloc_count] = n + 1u;
        ++ctx->alloc_count;
        ctx->regs[dst] = (uint64_t)(uintptr_t)copy;
        ctx->zero = (n == 0);
        return JE_OK;
    }
    if (strcmp(op, "PRINTS") == 0) {
        int r = parse_reg(trim(cursor));
        if (r < 0) {
            je_set_error("PRINTS expects a pointer register"); return JE_ERR_PARSE;
        }
        void *ptr = (void *)(uintptr_t)ctx->regs[r];
        if (!ptr) {
            je_set_error("PRINTS expects a non-null pointer"); return JE_ERR_PARSE;
        }
        size_t ai = exec_find_alloc(ctx, ptr);
        if (ai == SIZE_MAX) { je_set_error("PRINTS pointer is not JE-owned"); return JE_ERR_PARSE; }
        const char *str = (const char *)ptr;
        size_t remain = ctx->alloc_sizes[ai] - (size_t)((uintptr_t)str - (uintptr_t)ctx->allocs[ai]);
        const char *nul = (const char *)memchr(str, '\0', remain);
        if (!nul) { je_set_error("PRINTS string is not NUL-terminated"); return JE_ERR_PARSE; }
        fputs(str, stdout);
        fputc('\n', stdout);
        return JE_OK;
    }
...
JEStatus je_exec_validate_text(const char *code, size_t size) {
    if (!code) return JE_ERR_INVALID_ARGUMENT;
    JEProgram p;
...
JEStatus je_exec_text(JEExecContext *ctx, const char *code, size_t size) {
    ...
    ctx->alloc_count = 0; ctx->file_count = 0;
    while (!ctx->halted && ctx->ip < p.count) {
        if (++ctx->steps > ctx->max_steps) {
            free_program(&p);
            je_runtime_cleanup(ctx);
            je_set_error("JE execution step limit exceeded");
            return JE_ERR_UNSUPPORTED;
        }
        int ip_changed = 0;
        st = exec_line(ctx, &p, p.lines[ctx->ip], &ip_changed);
        if (st != JE_OK) { free_program(&p); je_runtime_cleanup(ctx); return st; }
